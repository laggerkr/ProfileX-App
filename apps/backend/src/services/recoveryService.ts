...
export async function createRecoveryBackup(db: AppDatabase, organizationId: string, actorId?: string) {
  const [profiles, proxies, settings] = await Promise.all([
    db.query("SELECT * FROM profiles WHERE organization_id=$1 ORDER BY created_at ASC", [organizationId]),
    db.query("SELECT * FROM proxies WHERE organization_id=$1 ORDER BY name ASC", [organizationId]),
    db.query("SELECT * FROM settings ORDER BY key ASC")
  ]);
  const backup = { version: 1, organizationId, createdAt: new Date().toISOString(), profiles, proxies, settings };
  await logActivity(db, "recovery.backup_created", `${organizationId}:${profiles.length}:${proxies.length}`, actorId);
  return backup;
}

export async function restoreRecoveryBackup(db: AppDatabase, backup: any, organizationId: string, actorId?: string) {
  ...
  await db.transaction(async (tx) => {
    ...
    for (const row of backup.profiles) await tx.exec(`INSERT INTO profiles (id,workspace_id,organization_id,name,tags,profile_group_id,notes,proxy_id,proxy_protocol,tab_behavior,operating_system,browser_engine,storage_mode,fingerprint,startup_urls,extensions,status,version,created_at,updated_at,last_launched_at,last_sync_at)
      ...
      ON CONFLICT (id) DO UPDATE SET workspace_id=EXCLUDED.workspace_id,organization_id=EXCLUDED.organization_id,name=EXCLUDED.name,tags=EXCLUDED.tags,profile_group_id=EXCLUDED.profile_group_id,notes=EXCLUDED.notes,proxy_id=EXCLUDED.proxy_id,proxy_protocol=EXCLUDED.proxy_protocol,tab_behavior=EXCLUDED.tab_behavior,operating_system=EXCLUDED.operating_system,browser_engine=EXCLUDED.browser_engine,storage_mode=EXCLUDED.storage_mode,fingerprint=EXCLUDED.fingerprint,startup_urls=EXCLUDED.startup_urls,extensions=EXCLUDED.extensions,status=EXCLUDED.status,version=EXCLUDED.version,updated_at=EXCLUDED.updated_at,last_launched_at=EXCLUDED.last_launched_at,last_sync_at=EXCLUDED.last_sync_at`, [row.id,row.workspace_id,organizationId,row.name,row.tags,row.profile_group_id,row.notes,row.proxy_id,row.proxy_protocol,row.tab_behavior,row.operating_system,row.browser_engine,row.storage_mode,row.fingerprint,row.startup_urls,row.extensions,row.status,row.version,row.created_at,row.updated_at,row.last_launched_at,row.last_sync_at]);
  });
  await logActivity(db, "recovery.backup_restored", `${organizationId}:${backup.profiles.length}:${backup.proxies.length}`, actorId);
  return { restored: true, profiles: backup.profiles.length, proxies: backup.proxies.length, settings: backup.settings.length };
}

export async function clearStaleSessions(db: AppDatabase, organizationId: string, actorId?: string) {
  const sessions = await db.query<any>("UPDATE active_sessions s SET ended_at=now() FROM profiles p WHERE s.profile_id=p.id AND p.organization_id=$1 AND s.ended_at IS NULL RETURNING s.id", [organizationId]);
  await db.exec("DELETE FROM profile_locks l USING profiles p WHERE l.profile_id=p.id AND p.organization_id=$1", [organizationId]);
  await logActivity(db, "recovery.sessions_cleared", `${organizationId}:${sessions.length}`, actorId);
  return { clearedSessions: sessions.length };
}

export async function fixRunningProfiles(db: AppDatabase, organizationId: string, actorId?: string) {
  const rows = await db.query<any>("UPDATE profiles SET status='ready',updated_at=now() WHERE organization_id=$1 AND status='running' AND id NOT IN (SELECT profile_id FROM profile_locks) RETURNING id", [organizationId]);
  await logActivity(db, "recovery.running_profiles_fixed", `${organizationId}:${rows.length}`, actorId);
