...
export async function cloneProfile(db: AppDatabase, id: string, actorId?: string, organizationId?: string) { const profile = await getProfile(db, id, organizationId); return profile ? createProfile(db, { ...profile, name: `${profile.name} Copy` }, actorId, organizationId) : undefined; }
