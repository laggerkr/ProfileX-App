...
router.get('/rdp',requirePermission(db,'rdp.read',permissionChecks.manageRdp),async(_q,r)=>r.json({data:await listRdpConnections(db,{includePassword:['owner','admin'].includes(r.locals.authUser.role)})})); router.post('/rdp',requirePermission(db,'rdp.create',permissionChecks.manageRdp),async(q,r)=>r.status(201).json({data:await createRdpConnection(db,q.body)})); router.patch('/rdp/:id',requirePermission(db,'rdp.update',permissionChecks.manageRdp),async(q,r)=>{const x=await updateRdpConnection(db,q.params.id,q.body); return x?r.json({data:x}):r.status(404).json({error:'RDP connection not found'})}); router.delete('/rdp/:id',requirePermission(db,'rdp.delete',permissionChecks.manageRdp),async(q,r)=>r.json({data:{deleted:await deleteRdpConnection(db,q.params.id)}})); router.post('/rdp/:id/launch',requireDesktopClient(),requirePermission(db,'rdp.launch',permissionChecks.manageRdp),async(q,r)=>{await assertLaunchAllowed(db,r.locals.authUser.organizationId); const x=await launchRdpConnection(db,q.params.id); return x?r.json({data:x}):r.status(404).json({error:'RDP connection not found'})});
